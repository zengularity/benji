...
object S3CephSpec extends org.specs2.mutable.Specification {
  ...
  "Ceph client" should {
    ...
    "Access the system" in running(app) { implicit ee: EE =>
      val s3 = ceph
    ...
    "Create buckets and files" in running(app) {
      implicit ee: EE =>
        val s3 = ceph
    ...
    s"Creating bucket $bucketName and get a list of all buckets" in running(
      app
    ) { implicit ee: EE =>
    ...
    s"Write file in $bucketName bucket" in running(app) {
      implicit ee: EE =>
    ...
    s"Get contents of $bucketName bucket" in running(app) {
      implicit ee: EE =>
    ...
    "Creating & deleting buckets" in running(app) {
      implicit ee: EE =>
        val s3 = ceph
        val name = s"bucket-${System identityHashCode s3}"
    ...
    "Get contents of a file" in running(app) { implicit ee: EE =>
    ...
    "Get contents of a non-existing file" in running(
      app
    ) { implicit ee: EE =>
    ...
    "Write and copy file" in running(app) { implicit ee: EE =>
